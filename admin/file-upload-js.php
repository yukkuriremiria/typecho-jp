...
$(document).ready(function() {
    ...
    function fileUploadError (error) {
        ...
        switch (code) {
            case plupload.FILE_SIZE_ERROR:
                word = '<?php _e('ファイルサイズが制限を超える'); ?>';
                break;
            case plupload.FILE_EXTENSION_ERROR:
                word = '<?php _e('ファイル拡張子はサポートされていません。'); ?>';
                break;
            case plupload.FILE_DUPLICATE_ERROR:
                word = '<?php _e('ファイルはすでにアップロードされています。'); ?>';
                break;
            case plupload.HTTP_ERROR:
            default:
                word = '<?php _e('アップロードのエラー'); ?>';
                break;
        }

        var fileError = '<?php _e('%s アップロード失敗'); ?>'.replace('%s', file.name),
            li, exist = $('#' + file.id);
    ...
    function fileUploadComplete (id, url, data) {
        var li = $('#' + id).removeClass('loading').data('cid', data.cid)
            ...
            .html('<input type="hidden" name="attachment[]" value="' + data.cid + '" />'
                + '<a class="insert" target="_blank" href="###" title="<?php _e('ファイルの挿入'); ?>">' + data.title + '</a><div class="info">' + data.bytes
                + ' <a class="file" target="_blank" href="<?php $options->adminUrl('media.php'); ?>?cid=' 
                + data.cid + '" title="<?php _e('コンパイラ'); ?>"><i class="i-edit"></i></a>'
                + ' <a class="delete" href="###" title="<?php _e('除去'); ?>"><i class="i-delete"></i></a></div>')
    ...
    var uploader = null, tabFilesEl = $('#tab-files').bind('init', function () {
        uploader = new plupload.Uploader({
            ...
            filters         :   {
                ...
                mime_types          :   [{'title' : '<?php _e('アップロード可能なファイル'); ?>', 'extensions' : '<?php echo implode(',', $options->allowedAttachmentTypes); ?>'}],
    ...
    function attachDeleteEvent (el) {
        ...
        $('.delete', el).click(function () {
            if (confirm('<?php _e('确认要除去文件 %s (質問タグ)?'); ?>'.replace('%s', file))) {
