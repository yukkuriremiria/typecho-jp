                ...
                <form method="post" name="manage_tags" class="operate-form">
                        ...
                        <div class="operate">
                            ...
                            <div class="btn-group btn-drop">
                                ...
                                <ul class="dropdown-menu">
                                    <li><a lang="<?php _e('これらのタグを削除してもよろしいですか？?'); ?>"
                                           href="<?php $security->index('/action/metas-tag-edit?do=delete'); ?>"><?php _e('除去'); ?></a>
                                    </li>
                                    <li><a lang="<?php _e('タブの更新に時間がかかる場合があります。, 本当にタブを更新しますか？?'); ?>"
                                           href="<?php $security->index('/action/metas-tag-edit?do=refresh'); ?>"><?php _e('リフレッシュ'); ?></a>
                                    </li>
                                    <li class="multiline">
                                        <button type="button" class="btn btn-s merge"
                                                rel="<?php $security->index('/action/metas-tag-edit?do=merge'); ?>"><?php _e('にマージする。'); ?></button>
                    ...
                    <ul class="typecho-list-notable tag-list clearfix">
                        <?php if ($tags->have()): ?>
                            ...
                            <?php endwhile; ?>
                        <?php else: ?>
                            <h6 class="typecho-list-table-title"><?php _e('レッテルはない。'); ?></h6>
                        <?php endif; ?>
                    </ul>
