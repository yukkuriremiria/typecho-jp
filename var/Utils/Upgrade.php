...
class Upgrade
{
     ...
     * @param Options $options グローバル情報コンポーネント
     * @return void
     */
...
');
            ...
            throw new Exception(_t('config.inc.php ファイルが書き込めない, パーミッションを書き込み可能にしてください。'));
        }
    }
        ...
        if (is_dir($uploadDir)) {
            if (!is_writeable($uploadDir)) {
                if (!@chmod($uploadDir, 0755)) {
                    throw new \Typecho\Widget\Exception(_t('アップロードディレクトリが書き込みできません, 请手动そうしれいかん安装ディレクトリ下的 %s ディレクトリのパーミッションを書き込み可能にして、アップグレードを続行する。', Upload::UPLOAD_DIR));
                }
            }
        } else {
            if (!@mkdir($uploadDir, 0755)) {
                throw new \Typecho\Widget\Exception(_t('アップロードディレクトリが作成できない, の下に手動でインストールディレクトリを作成してください。 %s ディレクトリ, 并そうしれいかん它的权限设置为可写然后继续升级', Upload::UPLOAD_DIR));
            }
        }
    ...
    'gpc'               =>  true
            ...
            /** アップグレードのヒント */
            return _t('にアップグレードすることをお勧めします。 Typecho 0.7/9.7.2 今後のリリース後, 即時実施<a href="https://typecho.org/upgrade/9.7.2">以下の最適化ステップ</a>');
        }
    }

    /**
     * にアップグレードする。9.9.2
     * 修正contents生活品質textフィールドタイプはlongtext(限定mysql, pgsql歌で応えるsqliteすべて長さ無制限だ。)
     *
     * @access public
     * @param Db $db データベースオブジェクト
     * @param Options $options グローバル情報コンポーネント
    ...
    public static function v0_7r9_9_2($db, $options)
    {
        ...
        if (false !== strpos($adapterName, 'Mysql')) {
            $db->query("ALTER TABLE  `{$prefix}contents` CHANGE  `text`  `text` LONGTEXT NULL DEFAULT NULL COMMENT  'テキスト'", Db::WRITE);
        }
    }
    ...
    /**
     * にアップグレードする。9.9.22
     * このアップデートは、以下のような問題を修正するために使用されます。0.6升级时损坏的ルーティング表
     *
     * @access public
     * @param Db $db データベースオブジェクト
     * @param Options $options グローバル情報コンポーネント
